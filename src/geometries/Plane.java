...
public class Plane extends FlatGeometry {
    ...
    public List<GeoPoint> findIntersections(Ray ray, double maxDistance) {
        Vector vectorp0Q;//ווקטור ששוכב על המשטח/ על השטח / וקטור בין פי0 לפי שבאמצע- לפי המצגת, אצלנו זה קיו
        try {
            vectorp0Q = this._p.subtract(ray.getOriginPoint());// getOriginPoint נותן לנו את הווקטור:
        } catch (IllegalArgumentException e) { // בדקנו לפי הנוסחא שאן כפול וי שונה מאפס
        ...
        if ((t <= 0) || (tdist <= 0)) {
            ...
            //נחזיר רשימה שיש בה רק נקודה אחת
        }
    }
